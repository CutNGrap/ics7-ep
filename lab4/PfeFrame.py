...
class PfeFrame(tk.Frame): 
    def __init__(self, master): 
        ...
        a_label = tk.Label(  self.formula_frame, text="Постоянная a: ")
        ...
        alpha_label = tk.Label(  self.formula_frame, text="Звездное плечо: ")
    ...
    def run(self, lambda_min, lambda_max, mu_min, mu_max, count, lambda2_min, lambda2_max, mu2_min, mu2_max):
        ...
        a = norm_to_nat_dfe(xm, xd, b[:11] + b[16:])

        x_indexes = ["0", "1", "2", "3", "4", 
             "12","13","14", "23", "24", "34", 
            #   "123","124","134", "234", "1234",
            '1^2', '2^2', '3^2', '4^2',]
        not_lin_str = "y = " + str('{:.5g}'.format(a[0]))
        for i in range (1, len(a)): 
            if i == round(len(a)/2):
                not_lin_str += '\n'
            if (b[i] > 0):
                not_lin_str += " + " + str('{:.6f}'.format(a[i])) + " * x" + x_indexes[i]
            else: 
                not_lin_str += " - " + str('{:.6f}'.format(math.fabs(a[i]))) + " * x" + x_indexes[i]
