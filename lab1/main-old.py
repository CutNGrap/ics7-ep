...
def work_proc(Event):
    ...
    result = modelling(
        ...
        generator_intensity=float(varList["lambda"].get()),
        processor_intensity=float(varList["mu"].get()),
        processor_range=0
    )
    ...
    messagebox.showinfo(title="Результаты",
                        message="Загрузка системы (расчетная): {:.4f}\nЗагрузка системы (фактическая): {:.4f}\nВремя работы:{:.4f} \nСреднее время ожидания: {:.4f}"
                        .format(workload, result['workload'], result['time'], result['wait_time_middle']))
...
def one_model_list(root):
    items = [
        i.Item(text="Интенсивность поступления заявок:",
               var=varList["lambda"], value=10),
        i.Item(text="Интенсивность обслуживания заявок:",
               var=varList["mu"], value=15),
