...
def read():
    ...
    time = 0
    try:
        params_generator = [1 / float(entry_mx_1.get()), 0.5]
        params_handler = [1 / float(entry_mx_2.get()), 0.5]
...
Label(text="(экспоненциальное распределение)").grid(row=0, column=2)
...
Label(text="(распределение Вейбулла с параметром 2)").grid(row=1, column=2)
